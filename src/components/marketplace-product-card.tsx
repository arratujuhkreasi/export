...
export function MarketplaceProductCard({
  ...
  return (
    <div className="marketplace-card group flex h-full flex-col overflow-hidden rounded-xl border border-border/60 bg-white shadow-sm">
      {/* Image */}
      <Link href={hrefWithLocale(`/products/${product.slug}`, locale)} className="block">
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        {/* Product name */}
        <Link href={hrefWithLocale(`/products/${product.slug}`, locale)} className="mt-1.5 block">
          <h3 className="line-clamp-2 text-sm font-semibold leading-5 text-foreground hover:text-[#1d6b4f]">
            {product.name}
          </h3>
        </Link>
